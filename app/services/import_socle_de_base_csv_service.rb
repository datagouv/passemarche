...
class ImportSocleDeBaseCsvService < ApplicationService
  ...
  def initialize(csv_file:, dry_run: false)
    @csv_file = csv_file
    @dry_run = dry_run
  ...
  def parse_csv
    rows = CSV.read(resolve_path, headers: true, col_sep: ';', liberal_parsing: true)
    validate_csv_format!(rows)
    rows
  ...
  def validate_csv_format!(rows)
    return if rows.empty?

    expected = COLUMN.size
    actual = rows.headers.size
    return unless actual < expected

    @errors << "Format CSV invalide : #{actual} colonnes détectées au lieu de #{expected}. " \
  ...
  def persist_row(row, key)
    ...
    collect_changes(attribute, was_new, row) if @dry_run
  ...
  def run_in_preview(rows)
    ActiveRecord::Base.transaction do
      rows.each_with_index { |row, index| import_single_row(row, index + 2) }
      preview_soft_deletes
      raise ActiveRecord::Rollback
    end
  end

  def preview_soft_deletes
    return if @imported_keys.empty?

    MarketAttribute
      .where.not(key: @imported_keys)
      .where(deleted_at: nil)
      .find_each do |attr|
        @changes[:deletes] << { key: attr.key, buyer_name: attr.buyer_name }
        @statistics[:soft_deleted] += 1
      end
  end

  def collect_changes(attribute, was_new, row)
    if was_new
      @changes[:creates] << { key: attribute.key, buyer_name: col(row, :buyer_name) }
    elsif attribute.changes.any?
      @changes[:updates] << {
        key: attribute.key,
        buyer_name: col(row, :buyer_name),
        diff: attribute.changes.transform_values { |from, to| { from:, to: } }
      }
    end
  end
  ...
  def build_result
    Result.new(success: @errors.empty?, statistics: @statistics, errors: @errors, changes: @changes)
  end
