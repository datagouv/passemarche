...
class FieldConfigurationImportService < ApplicationServiceObject
  ...
  def perform
    ...
    result = FieldConfigurationImport.call(context)

    if result.success?
      @result = result.statistics
    else
      add_error(:import, result.message || 'Import failed')
      @result = nil
    end
  end
end
