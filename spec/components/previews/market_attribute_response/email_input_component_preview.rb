...
class MarketAttributeResponse::EmailInputComponentPreview < Lookbook::Preview
  ...
  def form_manual_filled
    response = create_response(source: :manual, text: 'contact@entreprise.fr')
    render_with_form(response)
  end
  ...
  # @display bg_color "#f6f6f6"
  def form_auto
    response = create_response(source: :auto, text: 'dirigeant@api-entreprise.fr')
    render_with_form(response)
  end
  ...
  def form_manual_after_api_failure
    response = create_response(source: :manual_after_api_failure, text: 'fallback@exemple.fr')
    render_with_form(response)
  end
  ...
  def display_web_manual
    response = create_response(source: :manual, text: 'contact@entreprise.fr')
    render MarketAttributeResponse::EmailInputComponent.new(
  ...
  def display_web_auto
    response = create_response(source: :auto, text: 'valeur-automatique@cache.fr')
    render MarketAttributeResponse::EmailInputComponent.new(
      market_attribute_response: response,
      context: :web
    )
  ...
  def display_pdf_manual
    response = create_response(source: :manual, text: 'attestation@pdf.fr')
    render MarketAttributeResponse::EmailInputComponent.new(
      market_attribute_response: response,
      context: :pdf
    )
  ...
  def display_buyer_manual
    response = create_response(source: :manual, text: 'acheteur@visible.fr')
    render MarketAttributeResponse::EmailInputComponent.new(
      market_attribute_response: response,
      context: :buyer
    )
  ...
  def display_buyer_auto
    response = create_response(source: :auto, text: 'valeur-auto@visible-acheteur.fr')
    render MarketAttributeResponse::EmailInputComponent.new(
      market_attribute_response: response,
      context: :buyer
    )
  ...
  def create_response(source:, text:)
    market_attribute = MarketAttribute.find_or_initialize_by(key: 'preview_email_input_component') do |attr|
    ...
    response = MarketAttributeResponse::EmailInput.find_or_initialize_by(
      market_application:,
      market_attribute:
    )
