...
RSpec.describe MarketAttributeResponse::TextInputComponent, type: :component do
  ...
  describe '#text_value' do
    context 'with text present' do
      it 'returns the text value' do
        response = create(:market_attribute_response_text_input, market_attribute:, value: { 'text' => 'ACME Corp' })
        component = described_class.new(market_attribute_response: response)

        expect(component.text_value).to eq('ACME Corp')
      end
    end

    context 'with text blank' do
      it 'returns empty string' do
        response = build_stubbed(:market_attribute_response_text_input, market_attribute:, value: { 'text' => '' })
        component = described_class.new(market_attribute_response: response)
    ...
    context 'with nil value' do
      it 'returns empty string' do
        response = create(:market_attribute_response_text_input, market_attribute:, source: :auto, value: nil)
        component = described_class.new(market_attribute_response: response)
  ...
  describe '#display_value' do
    context 'with text present' do
      it 'returns the text value' do
        ...
        expect(component.display_value).to eq('ACME Corp')
      end
    end
  ...
  describe 'form mode' do
    ...
    context 'with manual source' do
      it 'renders input group with text field' do
        ...
        expect(response_form).to have_received(:text_field).with(:text, hash_including(class: 'fr-input'))
      end

      it 'includes required label class' do
        response = create(:market_attribute_response_text_input, market_attribute:, source: :manual, value: { 'text' => 'Test' })
        component = described_class.new(market_attribute_response: response, form:)
    ...
    context 'with auto source' do
      it 'renders hidden fields only' do
        response = create(:market_attribute_response_text_input, market_attribute:, source: :auto, value: { 'text' => 'Auto value' })
        component = described_class.new(market_attribute_response: response, form:)
    ...
    context 'with manual_after_api_failure source' do
      it 'renders input group with text field' do
        response = create(:market_attribute_response_text_input, market_attribute:, source: :manual_after_api_failure, value: { 'text' => 'Fallback' })
        component = described_class.new(market_attribute_response: response, form:)
    ...
    context 'with errors' do
      it 'adds error CSS classes to input group' do
        response = build_stubbed(:market_attribute_response_text_input, market_attribute:, source: :manual, value: { 'text' => '' })
      ...
      it 'adds error CSS classes to input field' do
        response = build_stubbed(:market_attribute_response_text_input, market_attribute:, source: :manual, value: { 'text' => '' })
        response.errors.add(:text, 'Ce champ est requis')
        component = described_class.new(market_attribute_response: response, form:)
      ...
      it 'displays error messages' do
        ...
        expect(page).to have_css('p.fr-error-text', text: 'Ce champ est requis')
      end
    end
  end
  ...
  describe 'display mode' do
    context 'with manual source and web context' do
      it 'shows field label and value' do
        response = create(:market_attribute_response_text_input, market_attribute:, source: :manual, value: { 'text' => 'ACME Corp' })
        ...
        expect(page).to have_css('span.fr-text--md.fr-text--mention-grey', text: 'ACME Corp')
      end
    end
    ...
    context 'with auto source and buyer context' do
      it 'shows field label and value for buyer' do
        ...
        expect(page).to have_css('span.fr-text--md.fr-text--mention-grey', text: 'Auto value')
      end
    end
    ...
    context 'with empty value' do
      it 'displays "Non renseigné"' do
        ...
        expect(page).to have_css('span.fr-text--md.fr-text--mention-grey', text: 'Non renseigné')
      end
    end
  end
  ...
  describe '#errors?' do
    ...
    it 'returns false when no errors' do
      response = create(:market_attribute_response_text_input, market_attribute:, value: { 'text' => 'Valid' })
      component = described_class.new(market_attribute_response: response)
