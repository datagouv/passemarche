...
RSpec.describe PublicMarketPresenter, type: :presenter do
  ...
  describe '#attributes_for_scope' do
    let(:works_type)    { create(:market_type, :works) }
    let(:services_type) { create(:market_type, :services) }
    ...
    let!(:works_only_attr) do
      attr = create(:market_attribute,
        key: 'works_only_attr',
        category_key: 'test_technical_capacity',
        subcategory_key: 'test_works_specific',
        mandatory: false)
      works_type.market_attributes << attr
      attr
    end

    let!(:services_only_attr) do
      attr = create(:market_attribute,
        key: 'services_only_attr',
        category_key: 'test_technical_capacity',
        subcategory_key: 'test_services_specific',
        mandatory: false)
      services_type.market_attributes << attr
      attr
    end
    ...
    it 'retourne les attributs communs aux deux types pour scope :commun' do
      result = multi_presenter.attributes_for_scope(:commun)
      expect(result.map(&:key)).to include('common_attr')
      expect(result.map(&:key)).not_to include('works_only_attr', 'services_only_attr')
    end

    it 'retourne uniquement les attributs works pour scope :works' do
      result = multi_presenter.attributes_for_scope(:works)
      expect(result.map(&:key)).to include('works_only_attr')
      expect(result.map(&:key)).not_to include('common_attr', 'services_only_attr')
    end

    it 'retourne uniquement les attributs services pour scope :services' do
      result = multi_presenter.attributes_for_scope(:services)
      expect(result.map(&:key)).to include('services_only_attr')
      expect(result.map(&:key)).not_to include('common_attr', 'works_only_attr')
    end

    it 'retourne un tableau vide pour un scope inconnu' do
      expect(multi_presenter.attributes_for_scope(:unknown)).to be_empty
    end
  end
