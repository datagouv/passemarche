...
RSpec.describe Fntp::DownloadDocument, type: :interactor do
  ...
  describe '.call' do
    subject { described_class.call(bundled_data:, params: { siret: }, api_name: 'fntp') }
    ...
    context 'when document is successfully downloaded' do
        ...
        stub_request(:get, document_url)
          .to_return(
            ...
            headers: {
              ...
              'Content-Disposition' => "attachment; filename=\"carte_professionnelle_tp_fntp_#{siren}.pdf\""
            }
          )
      end
      ...
      it 'uses FNTP-specific filename with SIREN' do
        result = subject
        expect(result.bundled_data.data.document[:filename]).to eq("carte_professionnelle_tp_fntp_#{siren}.pdf")
      end
    ...
    context 'with different SIRET values' do
      ...
      it 'extracts filename from URL path' do
        result = subject
        expect(result.bundled_data.data.document[:filename]).to eq("1569139162-#{siren}-carte_professionnelle.pdf")
      end
    end
    ...
    context 'when Content-Disposition header and URL filename are missing' do
      ...
      it 'generates filename using SIREN extracted from SIRET' do
        result = subject
        expect(result.bundled_data.data.document[:filename]).to eq("carte_professionnelle_tp_fntp_#{siren}.pdf")
      end
    end
