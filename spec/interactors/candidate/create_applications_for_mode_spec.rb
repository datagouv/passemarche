...
RSpec.describe Candidate::CreateApplicationsForMode, type: :interactor do
  ...
  describe '.call' do
    ...
    context 'when application_mode is mixte' do
      ...
      it 'creates a solo application and a distinct groupement application as the result' do
        result = subject

        solo_application = MarketApplication.where(public_market:, siret:).solo.sole
        expect(result.market_application).to be_groupement
        expect(result.market_application).not_to eq(solo_application)
      end
    ...
    context 'when the mandataire uniqueness Ruby validation is bypassed and only the database constraint remains (race condition safety net)' do
      ...
      before do
        other_application = create(:market_application, public_market:, siret:, application_mode: :groupement)
        create(:grouping, public_market:, mandataire_market_application: other_application)
        allow_any_instance_of(GroupingMember).to receive(:valid?).and_wrap_original do |original, *args|
          original.call(*args)
          true
        end
      end
      ...
      it 'does not leave a duplicate mandataire grouping behind' do
        subject
        expect(GroupingMember.mandataire.where(public_market:).count).to eq(1)
      end
    end
