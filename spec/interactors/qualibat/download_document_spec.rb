...
RSpec.describe Qualibat::DownloadDocument, type: :interactor do
  ...
  describe '.call' do
    ...
    context 'when document is successfully downloaded' do
      ...
      it 'adds Authorization header to the request' do
        subject
        expect(WebMock).to have_requested(:get, document_url)
          .with(headers: { 'Authorization' => "Bearer #{token}" })
      end

      it 'adds Accept header for PDF content' do
        subject
        expect(WebMock).to have_requested(:get, document_url)
          .with(headers: { 'Accept' => 'application/pdf, application/octet-stream, */*' })
      end
    end
    ...
    context 'when API token is missing' do
      ...
      it 'includes error message about missing credentials' do
