...
RSpec.describe Probtp, type: :organizer do
  ...
  describe '.call' do
    ...
    context 'when the API call and document download are successful' do
      ...
      before do
        stub_request(:get, api_url)
          ...
          .to_return(
            status: 200,
            body: probtp_attestation_success_response(siret:),
      ...
      it 'stores downloaded document as document hash' do
        ...
        expect(document[:filename]).to eq("1569139162-#{siret}-attestation_probtp.pdf")
      ...
      it 'sets api_name to probtp' do
        result = subject
        expect(result.api_name).to eq('probtp')
      end
    ...
    context 'when MakeRequest fails' do
      before do
        stub_request(:get, api_url)
          ...
          .to_return(status: 404, body: probtp_not_found_response)
      end
      ...
      it 'does not proceed to BuildResource' do
        expect(Probtp::BuildResource).not_to receive(:call)
        subject
      end
    end
    ...
    context 'when BuildResource fails' do
      ...
      it 'does not proceed to DownloadDocument' do
        expect(Probtp::DownloadDocument).not_to receive(:call)
        subject
      end
    end
