...
RSpec.describe FetchQualifelecDataJob, type: :job do
  ...
  describe '.api_name' do
    it 'returns qualifelec' do
      expect(described_class.api_name).to eq('qualifelec')
    end
  end

  describe '.api_service' do
    it 'returns Qualifelec' do
      expect(described_class.api_service).to eq(Qualifelec)
    end
  end

  describe '#perform' do
    context 'when Qualifelec API succeeds' do
      it 'calls the Qualifelec organizer with the correct parameters' do
        expect(Qualifelec).to receive(:call).with(
          params: { siret: },
          market_application:
    ...
    context 'when Qualifelec API fails' do
    ...
    context 'when Qualifelec API times out' do
