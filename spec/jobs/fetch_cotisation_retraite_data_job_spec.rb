...
RSpec.describe FetchCotisationRetraiteDataJob, type: :job do
  ...
  describe '.api_name' do
    it 'returns the correct API name' do
      expect(described_class.api_name).to eq('cotisation_retraite')
    end
  end

  describe '.api_service' do
    it 'returns the CotisationRetraite organizer' do
      expect(described_class.api_service).to eq(CotisationRetraite)
    end
  end
  ...
  describe '#perform' do
    context 'when market application has no SIRET' do
      ...
      it 'exits early without calling the API' do
        expect(CotisationRetraite).not_to receive(:call)
    ...
    context 'when API call is successful' do
      ...
      before do
        allow(CotisationRetraite).to receive(:call).and_return(successful_result)
      end
      ...
      it 'updates status to processing before API call' do
        ...
        expect(market_application).to have_received(:update_api_status)
          .with('cotisation_retraite', status: 'processing').ordered
      end

      it 'calls the CotisationRetraite organizer with correct parameters' do
        described_class.perform_now(market_application.id)

        expect(CotisationRetraite).to have_received(:call).with(
          params: { siret:, siren: },
          market_application:
        )
      end
      ...
      it 'updates status to completed after successful API call' do
        ...
        expect(market_application.api_fetch_status['cotisation_retraite']['status']).to eq('completed')
      end
    end
    ...
    context 'when API call fails' do
      ...
      before do
        allow(CotisationRetraite).to receive(:call).and_return(failed_result)
      end
      ...
      it 'updates status to failed' do
        ...
        market_application.reload
        expect(market_application.api_fetch_status['cotisation_retraite']['status']).to eq('failed')
      end
      ...
      it 'marks API attributes as manual_after_api_failure' do
        ...
        responses = market_application.market_attribute_responses
          ...
          .where(market_attributes: { api_name: 'cotisation_retraite' })
