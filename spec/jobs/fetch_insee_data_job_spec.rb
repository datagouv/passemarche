...
RSpec.describe FetchInseeDataJob, type: :job do
  ...
  describe '.api_name' do
    it 'returns the correct API name' do
      expect(described_class.api_name).to eq('insee')
    end
  end

  describe '.api_service' do
    it 'returns the Insee organizer' do
      expect(described_class.api_service).to eq(Insee)
    end
  end
  ...
  describe '#perform' do
    context 'when API call is successful' do
      ...
      before do
        allow(Insee).to receive(:call).and_return(successful_result)
      end
      ...
      it 'updates status to processing before API call' do
        ...
        expect(market_application).to have_received(:update_api_status)
          .with('insee', status: 'processing').ordered
      end

      it 'calls the Insee organizer with correct parameters' do
        described_class.perform_now(market_application.id)

        expect(Insee).to have_received(:call).with(
          params: { siret: },
          market_application:
        )
      end
      ...
      it 'counts filled fields correctly' do
        ...
        create(:market_attribute_response,
          market_application:,
          market_attribute: insee_attribute_1,
          source: :auto)
        create(:market_attribute_response,
          market_application:,
          market_attribute: insee_attribute_2,
          source: :auto)
      ...
      it 'records the timestamp of the update' do
        freeze_time do
          ...
          market_application.reload
          expect(market_application.api_fetch_status['insee']['updated_at'])
    ...
    context 'when API call fails' do
      ...
      before do
        allow(Insee).to receive(:call).and_return(failed_result)
      end
      ...
      it 'sets fields_filled to 0' do
        ...
        market_application.reload
        expect(market_application.api_fetch_status['insee']['fields_filled']).to eq(0)
      end
      ...
      it 'marks API attributes as manual_after_api_failure' do
        ...
        responses = market_application.market_attribute_responses
          ...
          .where(market_attributes: { api_name: 'insee' })
    ...
    context 'with concurrent job execution (race condition test)' do
      it 'handles concurrent updates with pessimistic locking' do
        ...
        allow(Insee).to receive(:call).and_return(double('Result', success?: true))
        ...
        market_application.reload
        expect(market_application.api_fetch_status['insee']).to be_present
        expect(market_application.api_fetch_status['insee']['status']).to eq('completed')
      end
    end
