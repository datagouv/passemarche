...
RSpec.describe ExportSocleDeBaseCsvService do
  ...
  describe '.call' do
    subject(:result) { described_class.call(market_attributes: attributes) }
    ...
    it 'generates filename with current date' do
      expect(result[:filename]).to eq("socle-de-base-#{Date.current}.csv")
    end

    it 'uses semicolon as separator' do
      lines = result[:csv_data].lines
