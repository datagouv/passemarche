...
RSpec.describe MarketAttributeResponse::RadioWithJustificationOptional, type: :model do
  let(:market_application) { create(:market_application) }
  let(:market_attribute) { create(:market_attribute, input_type: :radio_with_justification_optional) }
  ...
  describe 'JSON schema validation' do
    context 'with empty value (new record defaults to "no", documents optional)' do
      let(:value) { nil }

      # KEY DIFFERENCE: Valid without documents when "no" (optional documents)
      it { is_expected.to be_valid }
    end

    context 'with empty hash (defaults to "no", documents optional)' do
      let(:value) { {} }
  ...
  describe 'radio choice scenarios' do
    context 'when radio is "no" with no documents (VALID - KEY DIFFERENCE)' do
      let(:value) { { 'radio_choice' => 'no' } }

      # KEY DIFFERENCE: Documents are optional when "no"
      it { is_expected.to be_valid }
    ...
    context 'when radio is "no" with both text and documents (INVALID - text not allowed)' do
      let(:value) { { 'radio_choice' => 'no', 'text' => 'Some text' } }
      let(:file_blob) do
        ActiveStorage::Blob.create_and_upload!(
          io: StringIO.new('justification document'),
  ...
  describe 'data consistency validation' do
    ...
    context 'when radio is no without documents (VALID - KEY DIFFERENCE)' do
      it 'is valid' do
