...
RSpec.describe MarketAttributeResponse, type: :model do
  ...
  describe '#manually_filled?' do
    let(:response) { build(:market_attribute_response_text_input) }

    it 'returns true when source is manual' do
      response.source = :manual
      expect(response).to be_manually_filled
    end
    ...
    it 'returns false when source is auto' do
      response.source = :auto
      expect(response).not_to be_manually_filled
    end
  end

  describe '#should_display_badge?' do
    let(:response) { build(:market_attribute_response_text_input) }

    it 'returns true when source is auto' do
      response.source = :auto
      expect(response).to be_should_display_badge
    end
    ...
    it 'returns false when source is manual' do
      response.source = :manual
      expect(response).not_to be_should_display_badge
    end
  end

  describe '#from_api_source?' do
    let(:response) { build(:market_attribute_response_text_input) }

    it 'returns true when source is auto' do
      response.source = :auto
      expect(response).to be_from_api_source
    end

    it 'returns false when source is manual' do
      response.source = :manual
      expect(response).not_to be_from_api_source
    end

    it 'returns false when source is manual_after_api_failure' do
      response.source = :manual_after_api_failure
      expect(response).not_to be_from_api_source
    end
  end
