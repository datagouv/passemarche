...
RSpec.describe 'Candidate::ApplicationModes', type: :request do
  ...
  describe 'GET .../application_mode' do
  ...
  describe 'PATCH .../application_mode' do
  ...
  describe 'readonly navigation (application_mode already chosen)' do
    ...
    context 'regression: revisiting application_mode while the legal type is not chosen yet must not skip it' do
      before do
        market_application.update!(application_mode: :groupement)
        create(:grouping, public_market:, mandataire_market_application: market_application, legal_type: nil)
      end

      it 'still points the continue link to grouping_legal_type' do
        get wizard_step_path(market_application, :application_mode), params: { readonly: true }
      ...
      it 'allows navigating to grouping_legal_type to choose the type' do
        get wizard_step_path(market_application, :grouping_legal_type)

        rendered = Nokogiri::HTML(response.body)
        expect(rendered.at_css('fieldset')).to be_present
      end
    end
