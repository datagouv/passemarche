...
When('I remove the co-traitant {string}') do |siret|
  ...
  within("form[action='#{grouping_wizard_member_candidate_market_application_path(@market_application.identifier, member)}']") do
    click_button I18n.t('candidate.grouping_compositions.remove_button')
  end
end

When('I visit the grouping composition step directly') do
  visit grouping_wizard_step_candidate_market_application_path(@market_application.identifier, :grouping_composition)
end
