...
Feature: Admin Socle de Base - Créer un nouveau champ
  ...
  Background:
    ...
    And the following categories with labels exist:
      | key                 | buyer_label              |
      | identite_entreprise | Identité de l'entreprise |
    And the following subcategories with labels exist:
      | key            | category_key        | buyer_label    |
      | identification | identite_entreprise | Identification |
  ...
  Scenario: Access the creation form from the index
    ...
    And I should see the form block "Configuration de la source"
    And I should see the form block "Descriptions acheteur / candidat"
    And I should see the form block "Types de marché"
